...
/// to display a EULA the user must accept before the app does any work,
/// and to allow the user to re-display it later (we expose VEXEULA for
/// that). Acceptance is recorded per-user in a tiny marker file under
/// %APPDATA%\vex-bridge\ — the same dir the daemon already uses.
/// </summary>
internal static class Eula
{
    private const string EulaResourceName = "EULA.txt";

    /// <summary>
    /// If the user has already accepted, returns true with no UI.
    /// Otherwise shows the modal acceptance dialog and returns true on
    /// Accept, false on Decline. Callers should bail when this returns
    /// false — declining means do nothing.
    /// </summary>
    ...
    /// <summary>Always shows the dialog (used by VEXEULA).</summary>
    ...
    {
        // Try resource first; fall back to a baked-in short text so the
        // dialog never comes up empty on a partial install.
        try
        {
            using var s = typeof(Eula).Assembly.GetManifestResourceStream(
                $"Architur.VexBridgeAutoCAD.{EulaResourceName}");
            if (s is not null)
            {
...
Click Accept to continue, or Decline to cancel. You can re-open this
dialog later by typing VEXEULA at the AutoCAD command line.";
}
